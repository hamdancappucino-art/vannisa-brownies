...
export default function JurnalUmum() {
    ...
    API.get("/jurnal-umum")
      .then((res) => {
        const mapped = res.data.map((row) => ({
          ...row,
          nominal: row.debit > 0 ? row.debit : row.kredit,
          tipe_balance: row.debit > 0 ? "debit" : "kredit",
          kode_akun: row.kode_akun, // dari join coa
          ref_transaksi:
            row.id_transaksi ||
            row.id_dp ||
            row.id_beban ||
  ...
  const sortedData = filteredData
    .slice()
    .sort((a, b) => {
      const idA = parseInt(a.id_jurnal, 10) || 0;
      const idB = parseInt(b.id_jurnal, 10) || 0;
      return idA - idB;
    });
  ...
  function save() {
    ...
    const id = form.id_jurnal;

    API.put(`/jurnal-umum/${id}`, {
      ...
      .then(() => {
        ...
        showDialog({
          title: "Berhasil",
          subtitle: "Data jurnal umum berhasil diperbarui",
          type: "success",
        });
      })
      .catch((err) => {
  ...
  function remove(id) {
    showDialog({
      ...
      type: "warning",
      onConfirm: () => {
        API.delete(`/jurnal-umum/${id}`)
          .then(() => {
            setData((prev) =>
              prev.filter((item) => item.id_jurnal !== id)
            );
        ...
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <Card>
              {/* FILTER */}
              <SoftBox p={3}>
                <SoftTypography variant="h6">Jurnal Umum</SoftTypography>
